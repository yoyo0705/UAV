...
def main():
    ...
    for episode in range(EPISODES):
        ...
        while True:
            ...
            if done:
                ...
                rewars_list.append(episode_reward / step_counter)
                if result == 1:
                    result_list[0] += 1
                elif result == 2:
                    result_list[1] += 1
    ...
    plt.figure()
    plt.plot(range(len(losses_r)), list(losses_r), c='r', label='natural')
    # plt.plot(range(len(losses_b)), list(losses_b), c='b', label='double')
    plt.ylabel('Loss')
    plt.xlabel('training steps')
    plt.legend()
    plt.figure()
    plt.plot(list(range(len(q_evallist_r))), q_evallist_r, c='r', label='natural')
