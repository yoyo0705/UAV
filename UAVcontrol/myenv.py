...
class UAVEnv(gym.Env):
    ...
    def __init__(self, uav_num, map_w, map_h, map_z, Init_state):
        ...
        self.observation_space = spaces.Box(low=np.array([90, 0, 0, 0, -math.pi / 2, -math.pi] * self.uav_num),
    ...
    def step(self, actions):
        ...
        for i in range(self.uav_num):
            t = 0.25
            self.state[i][0] += t * (actions[i][0] - math.sin(self.state[i][4]))
        ...
        dd = [self.state[0][1] - self.state[1][1], self.state[0][2] - self.state[1][2] , self.state[0][3] - self.state[0][3]]
        d = np.linalg.norm(dd)
        AA = math.acos(vb @ dd / d)
        ATA = math.acos(vr @ dd / d)
        AAb = math.pi - ATA
        ATAb = math.pi - AA
    ...
    def render(self):
        def __init__(self, uav_num, state, map_w, map_h, map_z):
            self.uav_num = uav_num
            self.state = state
