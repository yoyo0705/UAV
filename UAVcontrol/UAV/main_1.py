...
def main():
    ...
    for episode in range(EPISODES):
        ...
        while True:
            ...
            action_b = netb.choose_action(state)
            action_r_array = np.array(action_r)
            action_b_array = np.array(action_b)
            action = np.stack([action_r_array, action_b_array])

            next_state, reward_r, done, _, _ = env.step(action)
            netr.store_trans(state, action_r, reward_r, next_state)  # 将状态转换存储到经验池中
            netb.store_trans(state, action_b, -reward_r, next_state)  # 将状态转换存储到经验池中

            env.recorder(step_counter)
            render.render3D()

            if netr.memory_counter >= MEMORY_CAPACITY:
                netr.learn()
                if done:
                    print("episode {}, the reward_r is {}, the reward_b is {}".format(episode, round(reward_r, 3),
                                                                                      round(reward_r, 3)))
            if done:
